	...
	function addAddress($address,$htmlMessage=null){
	 if (!is_string($address)){
		...
		if (empty($results['kml']['Response']['Placemark']['Point']['coordinates'])){
			...
			$pointer = count($this->validPoints);
			list($long,$lat) = explode(',',$results['kml']['Response']['Placemark']['Point']['coordinates']);
			$this->validPoints[$pointer]['lat']= $lat;
			$this->validPoints[$pointer]['long']= $long;
	...
	function showMap(){
    	...
    	function showmap(){
    		...
    		if (GBrowserIsCompatible()) {\n
             ...
             echo "map.centerAndZoom(new GPoint(".$this->defaultCenter('long').",".$this->defaultCenter('lat')."), ".$this->defaultZoomLevel().");\n";
             }else{
               echo $this->centerMap;
               }
		    echo "}\n
    ...
    function endElement($parser, $name)
     {
       ...
       if($key !='LatLonBox' && ($this->node_flag==1 || $key == 'Response' || $key == 'Placemark') ){
...
//////////////////END CODE FROM Roger Veciana's CLASS (assoc_array2xml) /////////////////////////////////
    ...
    function defaultCenter($direction)
    {
        //return $this->validPoints[0][$direction];
        $sum=0;
        foreach($this->validPoints as $point){
            $sum+=$point[$direction];
        }
        return $sum / sizeof($this->validPoints);
    }
    
    function defaultZoomLevel(){
        //return $this->zoomLevel;
        
        $deltaLat = $this->getDelta('lat');
        $deltaLong = $this->getDelta('long');
        if($deltaLat==0 && $deltaLong==0){
            return 4;
        }
        
        $max = abs($deltaLat);
        if(abs($deltaLong) > $max){
            $max = abs($deltaLong);
        }
        //log(.5)
        //log(44)
